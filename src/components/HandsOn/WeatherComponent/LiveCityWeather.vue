...
<template>
  <section class="live-city-weather">
    ...
    <p v-if="showHeading" class="description">
      전 세계 도시를 검색해 현재 날씨와 대기질을 확인해 보세요.
    </p>
    ...
    <p v-if="weatherStore.citySearchMessage" class="message">
      {{ weatherStore.citySearchMessage }}
    </p>
    ...
    <ul v-if="weatherStore.citySearchResults.length > 0" class="result-list">
      <li v-for="city in weatherStore.citySearchResults" :key="`${city.lat}-${city.lon}`">
        <button
          type="button"
          :disabled="weatherStore.isCitySearching"
          @click="selectSearchCity(city)"
        >
    ...
    <article v-if="showResult && weatherStore.selectedCityWeather" class="weather-result">
      <div class="result-title">
        ...
        <img
          v-if="weatherIconUrl"
          :src="weatherIconUrl"
          :alt="weatherStore.selectedCityWeather.weather[0].description"
        />
      </div>
      ...
      <div class="weather-values">
        <p>
          🌡️ 기온
          <strong
            >{{ formatTemperature(weatherStore.selectedCityWeather.main.temp)
            }}{{ unitSymbol }}</strong
          >
        </p>
        <p>
          🥵 체감
          <strong
            >{{ formatTemperature(weatherStore.selectedCityWeather.main.feels_like)
            }}{{ unitSymbol }}</strong
          >
        </p>
        <p>
          💧 습도 <strong>{{ weatherStore.selectedCityWeather.main.humidity }}%</strong>
        </p>
        <p>
          💨 풍속 <strong>{{ weatherStore.selectedCityWeather.wind.speed }}m/s</strong>
        </p>
        <p>
          🌫️ PM10
          <strong
            >{{ displayAirValue(weatherStore.selectedCityAirData?.components?.pm10) }}㎍/㎥</strong
          >
        </p>
        <p>
          😷 PM2.5
          <strong
            >{{ displayAirValue(weatherStore.selectedCityAirData?.components?.pm2_5) }}㎍/㎥</strong
          >
        </p>
      </div>
      ...
      <div v-if="weatherStore.selectedTravelInfo" class="travel-day-info">
        ...
        <p>
          일출 {{ formatTravelTime(weatherStore.selectedTravelInfo.daily.sunrise[0]) }} · 일몰
          {{ formatTravelTime(weatherStore.selectedTravelInfo.daily.sunset[0]) }}
        </p>
      </div>

      <div v-if="selectedCityItem" class="result-actions">
        <el-button type="danger" plain size="small" @click="saveSelectedCity('favorite')"
          >관심 여행지</el-button
        >
        <el-button
          type="primary"
          plain
          size="small"
          :disabled="tripDecisionStore.compareCount >= 2"
          @click="saveSelectedCity('compare')"
          >비교 도시</el-button
        >
