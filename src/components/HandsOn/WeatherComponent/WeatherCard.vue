...
// 날씨 계산은 섭씨 원본으로 유지하고 화면에 표시할 값만 Composable로 변환
const { unitSymbol, formatTemperature } = useTemperature()
const displayTemp = computed(() => formatTemperature(props.item.temp))
const displayFeelsLike = computed(() => formatTemperature(props.item.feelsLike))
...
<template>
  ...
  >
    ...
    <div class="card-heading">
      <div>
        <p class="continent">{{ item.country }} · {{ item.continent }}</p>
        <h4>{{ item.name }}</h4>
      </div>
    ...
    <div class="temperature-summary">
      ...
      <span
        >{{ item.status }}<small>체감 {{ displayFeelsLike }}{{ unitSymbol }}</small></span
      >
    </div>

    <div class="weather-info">
      <p>
        기온: <strong>{{ displayTemp }}{{ unitSymbol }}</strong>
      </p>
      <p>
    ...
    <!-- 선택한 여행 목적에 따른 안내 -->
    <div
      v-if="selectedPurpose !== '전체' && !purposeMatched"
      class="purpose-message purpose-failed"
    >
    ...
    <p v-else-if="selectedPurpose === '쾌적한 도시'" class="purpose-message">
      ✅ 비·온도·대기질 조건을 모두 만족한 도시입니다.
    </p>
    <p v-else-if="selectedPurpose === '더위 피하기'" class="purpose-message">
      🌿 체감온도 {{ displayFeelsLike }}{{ unitSymbol }}로 더위를 피하기 좋은 도시입니다.
    </p>
    <p v-else-if="selectedPurpose === '추위 피하기'" class="purpose-message">
      ☀️ 체감온도 {{ displayFeelsLike }}{{ unitSymbol }}로 비교적 따뜻한 도시입니다.
    </p>
    <p v-else-if="selectedPurpose === '비 피하기'" class="purpose-message">
      ☂️ 현재 비가 내리지 않는 도시입니다.
    </p>
    <p v-else-if="selectedPurpose === '대기질 좋은 곳'" class="purpose-message">
      🍃 미세먼지와 초미세먼지가 모두 좋은 단계입니다.
    </p>
    ...
    <div class="card-actions">
      <el-button
        ...
        size="small"
        @click.stop="tripDecisionStore.toggleFavorite(item.id)"
      >
        {{ isFavorite ? '♥ 관심 취소' : '♡ 관심 여행지' }}
      </el-button>
      <el-button
        :type="isCompareCity ? 'primary' : 'default'"
        size="small"
        :disabled="compareDisabled"
        @click.stop="tripDecisionStore.toggleCompare(item.id)"
      >
        {{ isCompareCity ? '✓ 비교 선택됨' : '⚖ 비교 도시' }}
      </el-button>
      <el-button type="primary" size="small" plain @click.stop="emit('click-detail', item)"
        >상세보기</el-button
      >
    </div>
  </el-card>
...
.purpose-mismatch-card {
  background: linear-gradient(160deg, #fffbeb, #fff 62%);
  border-color: #fbbf24;
  box-shadow: inset 4px 0 #f59e0b;
}
.searched-card {
  background: linear-gradient(160deg, #eff6ff, #fff 56%);
  border-color: #60a5fa;
}
.selected-card,
.selected-card.recommended-card,
.selected-card.searched-card {
  background: linear-gradient(160deg, #eff6ff, #fff 72%);
