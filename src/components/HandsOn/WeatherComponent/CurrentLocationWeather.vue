...
const displayTemp = computed(() => formatTemperature(weatherStore.currentWeather?.main?.temp))
const displayFeelsLike = computed(() =>
  formatTemperature(weatherStore.currentWeather?.main?.feels_like),
)
...
const outdoorMessage = computed(() => {
  ...
  if (air && (air.pm10 > 80 || air.pm2_5 > 35))
    return '😷 대기질이 좋지 않아 야외 활동에 주의하세요.'
  return '✈️ 현재 위치는 야외 활동하기 좋은 날씨입니다.'
})

const updatedTime = computed(() => {
  if (!weatherStore.lastUpdated) return ''
  return weatherStore.lastUpdated.toLocaleTimeString('ko-KR', {
    hour: '2-digit',
    minute: '2-digit',
  })
})
  ...
  <el-card class="location-weather" shadow="never">
    <div class="location-header">
      ...
      <el-button
        ...
        @click="weatherStore.fetchCurrentLocationWeather"
      >
    ...
    <el-alert
      ...
      :closable="false"
    >
      <el-button type="danger" size="small" @click="weatherStore.fetchCurrentLocationWeather"
        >다시 시도</el-button
      >
    </el-alert>
    ...
    <div v-else-if="weatherStore.hasCurrentWeather" class="weather-result">
      <div class="weather-title">
        <div>
          <h4>
            {{ weatherStore.currentLocation.name }} · {{ weatherStore.currentLocation.country }}
          </h4>
      ...
      <div class="weather-values">
        <p>
          🌡️ 기온 <strong>{{ displayTemp }}{{ unitSymbol }}</strong>
        </p>
        <p>
          🥵 체감 <strong>{{ displayFeelsLike }}{{ unitSymbol }}</strong>
        </p>
        <p>
          💧 습도 <strong>{{ weatherStore.currentWeather.main.humidity }}%</strong>
        </p>
        <p>
          💨 풍속 <strong>{{ weatherStore.currentWeather.wind.speed }}m/s</strong>
        </p>
        <p>
          🌫️ PM10 <strong>{{ weatherStore.airQualityData?.components?.pm10 ?? '-' }}㎍/㎥</strong>
        </p>
        <p>
          😷 PM2.5 <strong>{{ weatherStore.airQualityData?.components?.pm2_5 ?? '-' }}㎍/㎥</strong>
        </p>
      </div>

      <p class="air-quality">
        대기질: <strong>{{ aqiText }}</strong>
      </p>
      <p v-if="weatherStore.currentTravelInfo" class="travel-day-info">
        ☀️ 오늘 최대 자외선 {{ weatherStore.currentTravelInfo.daily.uv_index_max[0] }} · 일출
        {{ formatTravelTime(weatherStore.currentTravelInfo.daily.sunrise[0]) }} · 일몰
        {{ formatTravelTime(weatherStore.currentTravelInfo.daily.sunset[0]) }}
      </p>
...
.location-weather {
  height: 100%;
  background: linear-gradient(145deg, #eff6ff 0%, #fff 58%, #ecfeff 100%);
  border: 1px solid #bfdbfe;
  border-radius: var(--radius-md);
  box-shadow: var(--shadow-sm);
}
.location-weather :deep(.el-card__body) {
  display: flex;
  min-height: 100%;
...
.location-header :deep(.el-button) {
  min-height: 40px;
  flex: 0 0 auto;
  border-radius: 10px;
  font-weight: 700;
...
.location-weather :deep(.el-alert) {
  margin-top: 16px;
}
.weather-result {
  margin-top: 16px;
...
.weather-title img {
  ...
  height: 68px;
  flex: 0 0 68px;
  filter: drop-shadow(0 8px 10px rgba(14, 116, 144, 0.14));
}
.weather-values {
  display: grid;
